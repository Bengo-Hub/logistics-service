...
APP_NAME=${APP_NAME:-"logistics-service"}
NAMESPACE=${NAMESPACE:-"logistics"}
ENV_SECRET_NAME=${ENV_SECRET_NAME:-"logistics-service-secrets"}
DEPLOY=${DEPLOY:-true}
...
if ! kubectl -n "$NAMESPACE" get secret "$ENV_SECRET_NAME" >/dev/null 2>&1; then
  ...
  kubectl -n "$NAMESPACE" create secret generic "$ENV_SECRET_NAME" \
    --from-literal=LOGISTICS_POSTGRES_URL="postgresql://${SERVICE_DB_USER}:PASSWORD@postgresql.infra.svc.cluster.local:5432/${SERVICE_DB_NAME}?sslmode=disable" \
    --from-literal=LOGISTICS_REDIS_ADDR="redis-master.infra.svc.cluster.local:6379" \
    --from-literal=LOGISTICS_NATS_URL="nats://nats.messaging.svc.cluster.local:4222" || true
fi

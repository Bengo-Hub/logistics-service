...
# Build context should be from workspace root: docker build -f logistics-service/Dockerfile -t logistics-service:local .
COPY shared/auth-client /shared/auth-client
COPY logistics-service/go.mod logistics-service/go.sum ./
RUN go mod download
